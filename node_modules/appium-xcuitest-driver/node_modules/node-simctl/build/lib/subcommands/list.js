...
commands.getDevices = async function getDevices(forSdk, platform) {
    ...
    try {
        ...
        for (let [sdkName, entries] of lodash_1.default.toPairs(JSON.parse(stdout).devices)) {
            ...
            devices[sdk] = devices[sdk] || [];
            devices[sdk].push(...entries.filter((el) => lodash_1.default.isUndefined(el.isAvailable) || el.isAvailable)
                .map((el) => {
...
commands.getRuntimeForPlatformVersionViaJson = async function getRuntimeForPlatformVersionViaJson(platformVersion, platform = 'iOS') {
    ...
    for (const { version, identifier, name } of JSON.parse(stdout).runtimes) {
        if ((0, helpers_1.normalizeVersion)(version) === (0, helpers_1.normalizeVersion)(platformVersion)
