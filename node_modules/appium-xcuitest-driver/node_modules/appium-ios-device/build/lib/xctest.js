...
class Xctest {
    ...
    async _launchAppRunner(majorVersion, sessionIdentifier) {
        ...
        logger_1.default.info(`SignerIdentifier: ${signerIdentifier}`);
        ...
        logger_1.default.info(`Pid of launched ${this.xctestBundleId}: ${pid}`);
        ...
        try {
            ...
            for (const file of list) {
                if (file.endsWith(XCTEST_CONFIGURATION_EXTENSION)) {
                    ...
                    logger_1.default.debug(`removing ${fullPath}`);
    ...
    async _startExecSession(sessionIdentifier) {
        ...
        const showLogMessage = (message) => {
            if (message.auxiliaries.join('').includes('Received test runner ready reply with error: (null')) {
                logger_1.default.info('Test runner ready');
    ...
    async start() {
        if (this.running) {
            ...
            logger_1.default.info(`${message} Doing nothing here`);
            return;
        }
        this.running = true;
        try {
