...
async function findDeveloperImageFromDirectory(entry) {
    ...
    for (const subEntry of fileList) {
        ...
        if (subEntry.isDirectory()) {
            const fullPath = (0, path_1.join)(entry, subEntry.name);
...
async function findDeveloperImage(version, githubImageOption) {
    ...
    const DEFAULT_IMAGE_DIR = (0, path_1.join)(await support_1.env.resolveAppiumHome(), DEFAULT_IMAGE_DIR_NAME);
    const fullDownloadPath = (0, path_1.join)(DEFAULT_IMAGE_DIR, fileName);
    ...
    const decompressPath = (0, path_1.join)(DEFAULT_IMAGE_DIR, finalVersion);
    ...
    return {
        developerImage: (0, path_1.join)(developerImageParentFolder, DEVELOPER_IMAGE_FILE_NAME),
        developerImageSignature: (0, path_1.join)(developerImageParentFolder, DEVELOPER_IMAGE_SIGNATURE_FILE_NAME)
    };
}
/**
 *
...
async function searchAndDownloadDeveloperImageFromGithub(finalVersion, fullDownloadPath, githubImageOption) {
    ...
    try {
        logger_1.default.info(`Downloading developer image for ${finalVersion} to ${fullDownloadPath} from ${fileUrl}`);
