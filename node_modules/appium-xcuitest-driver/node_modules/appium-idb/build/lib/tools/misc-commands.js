...
miscCommands.describeDevice = async function describeDevice() {
    logger_js_1.default.debug(`Describing the device '${this.udid}'`);
...
miscCommands.focusSimulator = async function focusSimulator() {
    logger_js_1.default.debug(`Focusing Simulator '${this.udid}'`);
...
miscCommands.openUrl = async function openUrl(url) {
    logger_js_1.default.debug(`Opening URL '${url}' on the device '${this.udid}'`);
...
miscCommands.clearKeychain = async function clearKeychain() {
    logger_js_1.default.debug(`Clearing keychain on the device '${this.udid}'`);
...
miscCommands.setLocation = async function setLocation(latitude, longitude) {
    logger_js_1.default.debug(`Setting location (${latitude}, ${longitude}) on the device '${this.udid}'`);
...
miscCommands.approve = async function approve(bundleId, permissions) {
    logger_js_1.default.debug(`Approving permissions (${JSON.stringify(permissions)}) for '${bundleId}' ` +
        `on the device '${this.udid}'`);
    const command = ['approve'];
    const args = [];
    if (lodash_1.default.isArray(permissions)) {
...
miscCommands.addContacts = async function addContacts(dbPath) {
    logger_js_1.default.debug(`Adding contacts from (${dbPath}) to the device '${this.udid}'`);
    await this.exec(['contacts', 'update'], [dbPath]);
};
exports.default = miscCommands;
