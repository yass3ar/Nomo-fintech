...
function parseCssRule(cssRule) {
    ...
    const astClassNames = /** @type {import('css-selector-parser').AstClassName[]} */ (cssRule.items.filter(({ type }) => type === 'ClassName'));
    ...
    const astTag = /** @type {import('css-selector-parser').AstTagName|undefined} */ (cssRule.items.find(({ type }) => type === 'TagName'));
    ...
    const attrs = [];
    const astIds = /** @type {import('css-selector-parser').AstId[]} */ (cssRule.items.filter(({ type }) => type === 'Id'));
    const ids = astIds.map(({ name }) => name);
    if (ids.length) {
        attrs.push(`name == "${ids[0]}"`);
    }
    const attributes = /** @type {import('css-selector-parser').AstAttribute[]} */ (cssRule.items.filter(({ type }) => type === 'Attribute'));
    for (const attr of attributes) {
        attrs.push(parseAttr(attr));
    }
    const pseudoClasses = /** @type {import('css-selector-parser').AstPseudoClass[]} */ (cssRule.items.filter(({ type }) => type === 'PseudoClass'));
