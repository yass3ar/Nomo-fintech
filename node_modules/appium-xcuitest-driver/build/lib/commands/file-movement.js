...
async function parseContainerPath(remotePath, containerRootSupplier) {
    ...
    if (typeSeparatorPos > 0 && typeSeparatorPos < bundleId.length - 1) {
        ...
        logger_1.default.debug(`Parsed container type: ${containerType}`);
...
async function pushFileToSimulator(device, remotePath, base64Data) {
    ...
    if (CONTAINER_PATH_PATTERN.test(remotePath)) {
        const { bundleId, pathInContainer: dstPath } = await parseContainerPath(remotePath, async (appBundle, containerType) => await device.simctl.getAppContainer(appBundle, containerType));
        logger_1.default.info(`Parsed bundle identifier '${bundleId}' from '${remotePath}'. ` +
            `Will put the data into '${dstPath}'`);
        if (!(await support_1.fs.exists(path_1.default.dirname(dstPath)))) {
            logger_1.default.debug(`The destination folder '${path_1.default.dirname(dstPath)}' does not exist. Creating...`);
...
async function pullFromSimulator(device, remotePath, isFile) {
    ...
    if (CONTAINER_PATH_PATTERN.test(remotePath)) {
        ...
        logger_1.default.info(`Got the full item path: ${pathOnServer}`);
    }
    if (!(await support_1.fs.exists(pathOnServer))) {
        logger_1.default.errorAndThrow(`The remote ${isFile ? 'file' : 'folder'} at '${pathOnServer}' does not exist`);
    }
    const buffer = isFile
...
async function deleteFromSimulator(device, remotePath) {
    ...
    if (CONTAINER_PATH_PATTERN.test(remotePath)) {
        ...
        pathOnServer = path_1.default.posix.join(simRoot, remotePath);
        verifyIsSubPath(pathOnServer, simRoot);
        logger_1.default.info(`Got the full path: ${pathOnServer}`);
    }
    if (!(await support_1.fs.exists(pathOnServer))) {
