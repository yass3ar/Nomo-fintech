...
async function initPython({ pythonPath, dryRun = false, upgrade = false, } = {}) {
    ...
    if (dryRun) {
        dryRunLog.info('Would execute command: %s %s (environment variables: %s)', pythonPath, args.join(' '), constants_1.PIP_ENV_VARS);
    }
    else {
        log.debug('Executing command: %s %s (environment variables: %s)', pythonPath, args.join(' '), constants_1.PIP_ENV_VARS);
        log.info('Installing Python dependencies...');
        try {
            const result = await (0, teen_process_1.exec)(pythonPath, args, { env: constants_1.PIP_ENV_VARS, shell: true });
